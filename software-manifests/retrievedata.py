...
def retrieveDependencies(library):
    ...
    variables = {"ownerName": owner, "repoName": repo}
    ...
    depset = r.json(
    )["data"]["repository"]["dependencyGraphManifests"]["edges"]
        ...
        for dep in deps:
            ...
            if dep["node"]["repository"] is not None:
                tnode = SbomLibrary(
                    ...
                    f"https://github.com/{dep['node']['repository']['nameWithOwner']}",
                    unmaintained=dep["node"]["repository"]["isArchived"],
                    commitHash=dep["node"]["repository"]["defaultBranchRef"]
                    ["target"]["oid"],
